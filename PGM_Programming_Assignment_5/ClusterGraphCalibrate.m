...
for m = 1:length(edgeFromIndx),
    ...
    variable = intersect(P.clusterList(i).var, P.clusterList(j).var);
    MESSAGES(i, j).var = variable;
    MESSAGES(i, j).card = ones(1, length(variable)) * 2;
...
while (1),
    ...
    differ = setdiff(P.clusterList(i).var, sepset);
    MESSAGES(i, j) = FactorMarginalization(MESSAGES(i, j), differ);
        
    sum = 0;
    for k = 1:length(MESSAGES(i, j).val)
        sum = sum + MESSAGES(i, j).val(k);
    end
    for k = 1:length(MESSAGES(i, j).val)
        MESSAGES(i, j).val(k) = MESSAGES(i, j).val(k) / sum;
    end
