...
phenotypeFactor.card = ones(1, size(geneCopyVarOneList, 1) + size(geneCopyVarTwoList, 1) + 1) * 2;
...
for i = 1:size(geneCopyVarTwoList, 1)
    ...
    for j = 1:size(geneCopyVarOneList, 1)
        b = alleleWeights{1}(j);
        for m = 1:size(geneCopyVarTwoList, 1)
            c = alleleWeights{2}(m);
            for n = 1:size(geneCopyVarOneList, 1)
                d = alleleWeights{1}(n);
                value = computeSigmoid(a + b + c + d);
                index = (i - 1) * 8 + (j - 1) * 4 + (m - 1) * 2 + n;
                
                phenotypeFactor.val(index * 2 - 1) = value;
                phenotypeFactor.val(index * 2) = 1 - value;
            end
        end
    end
