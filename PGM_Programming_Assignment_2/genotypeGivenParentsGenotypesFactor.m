...
number = numAlleles * (numAlleles - 1) / 2 + numAlleles;
genotypeFactor.var = [genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo];
genotypeFactor.card = ones(1, 3) * number;
%genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

% Replace the zeros in genotypeFactor.val with the correct values.
probability = 1 / (numAlleles * numAlleles);
for i = 1:number
    for j = 1:number
        first = genotypesToAlleles(i, :);
        second = genotypesToAlleles(j, :);
        
        result = zeros(1, number);
        for m = 1:2
            for n = 1:2
                index = allelesToGenotypes(first(1, m), second(1, n));
                result(1, index) = result(1, index) + probability;
            end
        end
        genotypeFactor.val = [genotypeFactor.val, result];
    end
end
