...
for i = 1:numPeople
    ...
    if parents(1, 1) == 0 && parents(1, 2) == 0
        factorList(i).var = i;
        factorList(i).card = numAlleles;
        factorList(i).val = alleleFreqs';
    else
        factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, parents(1, 1), parents(1, 1) + numPeople);
    end
end

for i = numPeople + 1:numPeople * 2
    parents = pedigree.parents(i - numPeople, :);
    if parents(1, 1) == 0 && parents(1, 2) == 0
        factorList(i).var = i;
        factorList(i).card = numAlleles;
        factorList(i).val = alleleFreqs';
    else
        factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, parents(1, 2), parents(1, 2) + numPeople);
    end
end

for i = numPeople * 2 + 1:numPeople * 3
    factorList(i) = phenotypeGivenCopiesFactor(alphaList, numAlleles, i - numPeople * 2, i - numPeople, i);
end
