...
for i = 1:length(tripletList)
    index = AssignmentToIndex([tripletList(i).chars(1, 1), ...
        tripletList(i).chars(1, 2) , tripletList(i).chars(1, 3)], cardinality);
    value(index, 1) = tripletList(i).factorVal;
end

for i = 1:n - 2
    factors(i).var = [i, i + 1, i + 2];
    factors(i).card = cardinality;
    factors(i).val = value;
end
